...
const renderTimeline = () => {
  const list = query<HTMLElement>('.timeline-events');
  const card = query<HTMLElement>('.year-card');
  const scrubber = query<HTMLInputElement>('#year-scrubber');
  ...
  const update = (index: number) => {
    ...
    queryAll<HTMLButtonElement>('.timeline-dot', list).forEach((button, buttonIndex) => {
  ...
  list.addEventListener('click', (event) => {
    const button = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-year-index]');
    if (!button) return;
...
const renderFacilities = () => {
  query<HTMLElement>('.facility-grid').innerHTML = facilities
    .map(
      (facility) => `
...
const setupReveal = () => {
  ...
  queryAll<HTMLElement>('[data-reveal], .facility-card, .history-image, .lens-card').forEach((element) => {
...
const setupProgress = () => {
  ...
  const dockTitle = query<HTMLElement>('[data-current-section]');
  const dockCue = query<HTMLElement>('[data-current-cue]');
